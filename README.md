...
버그
- 1. desktop에서 실행(바로가기) 했을 때 desktop에서 정리된 폴더와 로그 파일이 남지 않는 버그 발생
- 2. 정리된 폴더에서 위치를 수정할 경우 되돌리기가 되지 않는 버그 발생
- 3. 지정된 폴더 목록 읽기 후 지정된 폴더를 옮기면 버그 발생

버그 해결 방향
- 1. desktop 폴더를 지정 할 수 없게 만들거나 desktop을 지정하더라도 바로가기에 해당하는 본 프로그램은 제외되게 만들어 버그가 없게 만들기
- 2. 정리된 폴더에서 위치를 수정하면 로그에 추가 되게 만들 수 있으면 좋음
- 3. 지정된 폴더 목록 읽기 후 지정된 폴더를 옮길 수 없게 프로세스로 만들어 강제로 옮기지 못하게 만들기
